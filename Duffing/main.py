...
class SpectralConv1d(nn.Module):
    ...
    # def compl_mul1d(self, lambda1,alpha, weights_pole, weights_residue):
    ...
    #     for le in range(lambda1.shape[0]):
    #         term1=torch.div(1,torch.sub(lambda1[le],weights_pole))
    #         Hw[:,:,:,le]=weights_residue*term1
    #         term2=torch.div(1,torch.sub(weights_pole,lambda1[le]))
    #         Pk[:,:,:,le]=weights_residue*term2
    #     output_residue1=torch.einsum("bix,iokx->box", alpha, Hw) 
    #     output_residue2=torch.einsum("bix,iokx->bok", alpha, Pk) 
    ...
    def compl_mul1d(self, lambda1,alpha, weights_pole, weights_residue):   
        ...
        # term2=torch.div(1,torch.sub(weights_pole,lambda1))
        # Pk=weights_residue*term2
        Pk=-Hw  
    ...
    def forward(self, x):
        ...
        output_residue1,output_residue2= self.compl_mul1d(lambda1, alpha, self.weights_pole, self.weights_residue)
        #stop=time.time()
        #time_req=stop-start
        #print(time_req)
...
summary(model,(2048,1),device="cuda")
...
# pred = torch.zeros(batch_size_test,s)
# test_l2 = 0.0
...
#         t=grid_x_test.cuda()
#         out = model(t,x)
#         pred= out.view(batch_size_test, -1)
#         test_l2 += myloss(out.view(batch_size_test, -1), y.view(batch_size_test, -1)).item()
